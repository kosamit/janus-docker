#!/bin/bash
# 
# https://github.com/mvnrsa/janus-combine-video/blob/master/janus-combine.sh
# janus-combine.sh
...
for FILE in $FILES
do
  /opt/janus/bin/janus-pp-rec $FILE-video.mjr $FILE-video.webm
  if [ $FILE-audio.mjr ]
  then
    /opt/janus/bin/janus-pp-rec $FILE-audio.mjr $FILE-audio.opus
